...
for i=1,#buildings,1 do
	setObjectInfo(buildings[i].cObj, IG3D_GRAVITY_MULTIPLIER, 1)
	setObjectInfo(buildings[i].cObj, IG3D_MASS, 0,0,0,0)
	
	setObjectInfo(buildings[i].cObj, IG3D_CAST_SHADOW, gDoShadows, computeShadowModelName(buildings[i]))
	buildings[i].house=true
	buildings[i].alive=false
	buildings[i].team=4
	buildings[i].life=1000
end
