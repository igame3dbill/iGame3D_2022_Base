gDangerFlag=false
-- handle_ZFPSAI copied from handle_FPSAI in FPS_Functions/FPS_AI.lua
		...
		for j=gAISliceOffset, gAISliceOffset + #gObjects-1, 1 do
			...
			if gObjects[i].alive then
				if numSensingPeople < gAISliceSize then
					...
					--this is for making sure the player is not AI'ed
					if i~=playerBox then
...
function zfps_zombies_in_touch_distance(z1, z2)
...
function zfps_build_zombie_near_values()
	local i,j
		...
		for i=1,#gObjects,1 do
			for j=1,#nearSet,1 do
				if gObjects[i].team==1 and gObjects[i].alive and not gObjects[i].nearPlayer then
					...
					local result= zfps_zombies_in_touch_distance(gObjects[i], nearSet[j])
					if result then
...
function zfps_enemy_in_sight(obj)
...
function zfps_zombies_follow()
	zfps_build_zombie_near_values()
	if gPathsSet then
...
function coll_obj_mesh_zfps(obj, bone, x,y,z)
