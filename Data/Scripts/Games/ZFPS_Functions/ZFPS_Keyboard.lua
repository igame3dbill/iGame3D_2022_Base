...
function zfps_game_keyboard()
--CMD Or CNTRol O to open ZFPS_Options 	
...
if key("m","n") then
	...
	setSound_emitterInfo(gZFPSMusic, IG3D_PAUSE, not gMusicOn)
	setSound_emitterInfo(gZFPSMusicAction, IG3D_PAUSE, not gMusicOn)
end

--all rush

if key("k","n") then
	gCurPlayerGun=3-gCurPlayerGun
end
...
if key("r","n") and gNumHumans>0 then
	...
	for i=1,#gObjects,1 do
		if gObjects[i].team==1 and gObjects[i].alive and gObjects[i].task.id~=object_action_eat then
			gObjects[i].delayedRush=time()+math.random()*0.5
			if not sndPlayed then zfps_play_zombie_acknowledge_sound(gObjects[i]); sndPlayed=true end
		end
	end
	rts_update_living_and_dead_counts()
end
...
--all hold position
if key("h","n") or (key(" ","n")  and gNextZFPSLevel==nil) then
...
if key(" ","n") and gNextZFPSLevel ~= nil then
	gDisplayLoading= true
	gLoadingBoxMessage="Loading"
	zfps_loading_text(gLoadingBoxMessage)
end
...
key_462 =commandKey
	...
	if key("+","n") then		a11,a12,a13,a14,a21,a22,a23,a24,a31,a32,a33,a34,a41,a42,a43,a44=getCameraInfo(IG3D_ROTATION_MATRIX)
		throw_grenade(a41,a42,a43,a31,a32,a33)
	end
