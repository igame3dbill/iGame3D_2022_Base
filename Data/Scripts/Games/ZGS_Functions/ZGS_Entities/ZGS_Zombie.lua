...
for i=1,#gObjects,1 do
	...
	if checkName=="zomb" or checkName=="reco" or checkName=="tank" then
--print(getObjectInfo(gObjects[i].cObj, IG3D_NAME))
		...
		gObjects[i].zombieSndEmt=assign_free_sound_emitter()
		setSound_emitterInfo(gObjects[i].zombieSndEmt, IG3D_SAMPLE, "Data/Sounds/zombie.ogg")
		setSound_emitterInfo(gObjects[i].zombieSndEmt, IG3D_VOLUME, 3)
		
		if string.sub(getObjectInfo(gObjects[i].cObj, IG3D_NAME),1,11)=="zombiechick" then
			setSound_emitterInfo(gObjects[i].zombieSndEmt, IG3D_PITCH, 200)
		end
		
		setSound_emitterInfo(gObjects[i].zombieSndEmt, IG3D_LOOP, true)
