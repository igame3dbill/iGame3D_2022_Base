...
-- open level
function openLevel()
fileName = fltk.fl_file_chooser("Choose a Level", "Lua Files (*.{lua})", nil, nil)
end
...
print(xPick,yPick)
print("mPick="..mPick)
if xPick=="Save" or xPick =="Export" then fileMenusavefile(yPick)
elseif   xPick== "Open"  or xPick == "Load" then fileMenuopenfile(yPick)
  end  
end


-- open file from menus 
 function fileMenuopenfile(yPick)
 gLastMenuPick=yPick
...
lua_icon:load(gameroot.."Data/Icons/FileIcons/WTFICON.png")
 ...
 local tDir=gameroot.."Data/"
local tFile="empty"
      if yPick=="default" then action = resetToDefault()
   	elseif yPick=="Level" then UniversalBrowse(yPick.."s")  
   	elseif yPick=="Script" then tFile =  chooseFile(tTitle,"*.lua",tDir.."Scripts/")
   	elseif yPick=="Fluid" then UniversalBrowse(yPick)
   	elseif yPick=="Mesh" then  OPEN_UI_WINDOWS("WTFbrowser")
   	elseif yPick=="Shader" then tFile =  chooseFolder(tTitle,tDir.."Shaders/") 
   	elseif yPick=="Object" then OPEN_UI_WINDOWS("WTFbrowser")
  	 ...
  	 if  tFile == nil or tFile == "empty" then  	return end
  	 
  	 tFile = string.gsub(tFile,"//","") 
  	 lastLoadedFile = string.sub(tFile,1,-5)
  	 print(lastLoadedFile)
  	 	if yPick=="Level" then loadLevel(tFile);
   		--elseif yPick=="Fluid" then  OPEN_UI_WINDOWS(item(-2,tFile,"/")) ; --see Data/Scripts/Default/Base/file_functions.lua
   		...
   		OPEN_UI_WINDOWS("script_editor"); --see Data/Scripts/Default/Base/file_functions.lua
   ...
   openfile=fileMenuopenfile
   
   -- save files from menus
function fileMenusavefile(yPick)
local tDir=gameroot.."Data/"
local tFile="empty"
   ...
   elseif yPick=="WTF" then OPEN_UI_WINDOWS("ExportWTF"); --see Data/Scripts/Default/Base/file_functions.lua
   elseif yPick=="OBJ" then save_asOBJ()
   elseif yPick=="3DS" then save_as3DS()
   elseif yPick=="Meshwork" then save_asMeshwork()
   end 
 print("fileMenusavefile"..yPick)
end
