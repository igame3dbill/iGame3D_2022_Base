...
function exportToObj(thefile1,thefile2)
	delete(ig3d_scene)
	ig3d_SetMode_i(1)
	loadMesh(thefile1)
	ig3d_SetMeshPositionNoUndo_3f(0,0,0)
	ig3d_SaveMeshAsOBJ_sf(thefile2,100)
	ig3d_SetMode_i(4)
	delete(ig3d_scene)
end

function batchWTFtoOBJ(thefolder1, thefolder2)
	local allFiles=listFiles(gameroot..thefolder1)
	local i
	local suffix
	...
	for i=1,#allFiles,1 do
		...
		print(allFiles[i])
		if suffix==".wtf" then
			exportToObj(thefolder1..allFiles[i], folder2...string.sub(allfiles[i],1,-5)..".obj")
		end
	end
end
