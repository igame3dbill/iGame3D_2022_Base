...
loadMesh("Data/Levels/SquigLand/badmesh.wtf")
...
function printfeld(f)
	local s=""
	for i=1,#f,1 do
		if s=="" then s=f[i] else s=s..","..f[i] end;
	end
	print(s)
end

function komb_rek(f,offset)
	local i
	for i=offset,#f,1 do
		...
		feld_kop[offset]=f[i]
		f[i]=feld_kop[i]
		f[offset]=feld_kop[offset]

		komb_rek(f,offset+1)
		if offset==#f then printfeld(feld_kop) end
		a=f[i]
		f[i]=f[offset]
		f[offset]=a
	end
